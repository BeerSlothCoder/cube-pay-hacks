...
-- FIXED: Changed alias from 'do' (reserved keyword) to 'obj'
...
BEGIN
    ...
    SELECT 
        obj.id,
        obj.agent_name,
        (obj.screen_position->>'x')::DECIMAL as screen_x,
        (obj.screen_position->>'y')::DECIMAL as screen_y,
        (obj.screen_position->>'z_index')::INTEGER as z_index,
        obj.model_url,
        obj.payment_enabled
    FROM deployed_objects obj
    WHERE 
        (obj.screen_position->>'x')::DECIMAL BETWEEN (center_x - view_width/2) AND (center_x + view_width/2)
        AND (obj.screen_position->>'y')::DECIMAL BETWEEN (center_y - view_height/2) AND (center_y + view_height/2)
        AND obj.is_active = true
    ORDER BY (obj.screen_position->>'z_index')::INTEGER DESC;
END;
$$ LANGUAGE plpgsql;
